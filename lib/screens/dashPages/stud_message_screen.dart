...
class MessageScreenStu extends StatefulWidget {
  final String fullname, stutoken;
  final int id;
  const MessageScreenStu({super.key, required this.fullname, required this.stutoken, required this.id});

  @override
  State<MessageScreenStu> createState() => _MessageScreenStuState();
}

class _MessageScreenStuState extends State<MessageScreenStu> {
  ...
  Future<void> loadData() async {
    try {
      await chat2(widget.id.toString(), widget.stutoken.toString());
      setState(() {
  ...
  chat1(String internshipApplicationId, message, message_from, token) async {
    try{
  ...
  Future<void> chat2(String id, token) async {
    try{
      final response = await get(
        Uri.parse('${ApiEndPoints.baseUrl}getChatByInternshipApplicationId/$id/S$token'),
      );
            ...
            children: [
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  onPressed: () async {
                    await chat1(widget.id.toString(), _text1Controller.text, 'S', widget.stutoken);
                    await loadData();
...
class MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final time = '${createdAt.split(' ')[1].split(':')[0]}:${createdAt.split(' ')[1].split(':')[1]}'; // Extract the time from the timestamp
