...
class ScheduleScreenUpEmp extends StatefulWidget {
  final int id, currentId;
  final int? interviewId;
  final String stuname, internshipDetails, emptoken;
  const ScheduleScreenUpEmp({super.key, required this.id, required this.internshipDetails, required this.stuname, required this.emptoken, required this.currentId, required this.interviewId});

  @override
  State<ScheduleScreenUpEmp> createState() => _ScheduleScreenUpEmpState();
}

class _ScheduleScreenUpEmpState extends State<ScheduleScreenUpEmp> {
  ...
  String interviewwType = '1';
  String? interviewTitle;
  String? interviewDescription;
  DateTime interviewDatetime = DateTime.now();
  String? interviewAddress;
  String? interviewLink;
  String? interviewContactno;
  String interviewTypeDesc = '';
  String interviewDurationDesc = '';

  @override
  void initState() {
    super.initState();
    getInt(widget.interviewId.toString()).then((_) {
      _titleController.text = interviewTitle ?? '';
      _addressController.text = interviewAddress ?? '';
      _contactController.text = interviewContactno ?? '';
      _descriptionController.text = interviewDescription ?? '';
      selectedDuration = interviewDurationDesc == '30 Mins' ? '30 Minute' : (interviewDurationDesc == '60 Mins' ? '60 Minute' : interviewDurationDesc);
      selectedDate = DateTime(interviewDatetime.year, interviewDatetime.month, interviewDatetime.day);
      selectedTime = TimeOfDay(hour: interviewDatetime.hour, minute: interviewDatetime.minute);
      _selectedType = interviewTypeDesc;
      _linkController.text = interviewLink ?? '';
    });
  }
  ...
  getInt(String id) async {
    try {
      final response = await get(
        Uri.parse('${ApiEndPoints.baseUrl}getInterviewScheduleById/$id'),
      );
      if (response.statusCode == 200) {
        setState(() {
          var data = jsonDecode(response.body.toString());
          //print(data);
          Map<String, dynamic> interviewData = data[0];
          interviewTitle = interviewData['interview_title'];
          interviewDescription = interviewData['interview_description'];
          interviewDatetime = DateTime.parse(interviewData['interview_datetime']);
          interviewAddress = interviewData['interview_address'];
          interviewLink = interviewData['interview_link'];
          interviewContactno = interviewData['interview_contactno'];
          interviewTypeDesc = interviewData['interview_type_desc'];
          interviewDurationDesc = interviewData['interview_duration_desc'];
        });
      } else {
        var data = jsonDecode(response.body.toString());
        print(data);
